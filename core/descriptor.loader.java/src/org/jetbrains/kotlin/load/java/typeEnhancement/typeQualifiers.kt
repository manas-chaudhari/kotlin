...
class JavaTypeQualifiers internal constructor(
        ...
        internal val additionalJavaTypeInfo: AdditionalJavaTypeInfo?
) {
    companion object {
        val NONE = JavaTypeQualifiers(null, null, null)
    }
}

internal class AdditionalJavaTypeInfo(val isNotNullTypeParameter: Boolean)
...
private fun KotlinType.extractQualifiers(): JavaTypeQualifiers {
    ...
    return JavaTypeQualifiers(
            ...
            if (getCapability<CustomTypeVariable>() is NotNullTypeParameterTypeCapability)
                AdditionalJavaTypeInfo(isNotNullTypeParameter = true)
            else
                null
    )
}
...
private fun KotlinType.extractQualifiersFromAnnotations(): JavaTypeQualifiers {
    ...
    return JavaTypeQualifiers(
            ...
            if (nullability == NOT_NULL && isTypeParameter())
                AdditionalJavaTypeInfo(isNotNullTypeParameter = true)
            else
                null
    )
...
private fun KotlinType.computeQualifiersForOverride(fromSupertypes: Collection<KotlinType>, isCovariant: Boolean): JavaTypeQualifiers {
    ...
    val isAnyNonNullTypeParameter = own.additionalJavaTypeInfo?.isNotNullTypeParameter ?: false || fromSupertypes.any {
        it.extractQualifiers().additionalJavaTypeInfo?.isNotNullTypeParameter ?: false
    }

    fun createJavaTypeQualifiers(nullability: NullabilityQualifier?, mutability: MutabilityQualifier?): JavaTypeQualifiers {
        if (!isAnyNonNullTypeParameter || nullability != NOT_NULL) return JavaTypeQualifiers(nullability, mutability, null)
        return JavaTypeQualifiers(
